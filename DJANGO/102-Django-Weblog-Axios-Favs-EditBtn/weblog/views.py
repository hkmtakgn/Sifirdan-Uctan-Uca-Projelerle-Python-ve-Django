...
def global_posts(request):
    g_posts = Post.objects.filter(is_active=True,
                                  user__is_online=True).order_by("-id")
...
def fav_update_views(request):
    ...
    if request.method == "POST":
        ...
        fav_post = get_object_or_404(Post, slug=slug, user=user)
...
def favorites(request):
    title = "Favorites"
    user = get_object_or_404(Interface_user, is_online=True)
    ids = user.favpost_set.filter(is_active=True).values_list(
        "post_id", flat=True).order_by("-id")
    fav_posts = Post.objects.filter(id__in=ids, is_active=True)
    context = dict(page_title=title, fav_posts=fav_posts)
    return render(request, "pages/favorites.html", context)


def fav_edit_views(request, fav_slug):
    title = "Edit Post"
    user = get_object_or_404(Interface_user,
                             is_online=True,
    ...
    fav_post = get_object_or_404(Post,
                                 ...
                                 is_active=True)
    if request.method == "POST":
        post_form = PostForm(request.POST, request.FILES, instance=fav_post)
        if post_form.is_valid():
            post_form.save()
            messages.info(request, "Post is saved!")
            return redirect("/")
    else:
        post_form = PostForm(instance=fav_post)
        context = dict(
            post_form=post_form,
            page_title=title,
        )
