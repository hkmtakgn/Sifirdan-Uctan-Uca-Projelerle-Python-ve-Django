...
def favorites(request):
    title = "Favorites"
    user = get_object_or_404(Interface_user, is_online=True)
    ids = user.favpost_set.all().values_list("id")
    fav_posts = FavPost.objects.filter(is_active=True, id__in=ids)
    context = dict(
        page_title=title,
        fav_posts=fav_posts,
    )
...
def edit_post(request, post_slug):
    ...
    if request.method == "POST":
        ...
        post_form = PostForm(request.POST, request.FILES, instance=edit_post)
        if post_form.is_valid():
            post_form.save()
    ...
    context = dict(page_title=title, post_form=post_form)
    return render(request, "pages/share-post.html", context)
