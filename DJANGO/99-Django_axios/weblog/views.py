...
def fav_update_views(request):
    if request.method == "POST":
        fav_post = get_object_or_404(Post, slug=request.POST.get("slug"))
        user = get_object_or_404(Interface_user, is_online=True)
        fav_set, created = FavPost.objects.get_or_create(post=fav_post,
                                                         user=user)
...
def favorites(request):
    title = "Favorites"
    fav_posts = FavPost.objects.filter(is_active=True)
    context = dict(
        page_title=title,
        fav_posts=fav_posts,
    )
